...
export const THEME_DECOS: Record<string, DecoDef[]> = {
  ...
  christmas: [
    { type:'snowflake', x:5,  y:6,  size:28, color:'rgba(255,255,255,0.7)', anim:'snow-drift 4s ease-in-out infinite',   delay:'0s',    opacity:0.85, rotate:0 },
    { type:'snowflake', x:75, y:4,  size:20, color:'rgba(255,255,255,0.6)', anim:'snow-drift 3.5s ease-in-out infinite', delay:'0.8s',  opacity:0.7,  rotate:30 },
    { type:'snowflake', x:40, y:8,  size:14, color:'rgba(255,255,255,0.5)', anim:'snow-drift 5s ease-in-out infinite',   delay:'1.5s',  opacity:0.55, rotate:15 },
    ...
    { type:'snowflake', x:88, y:82, size:16, color:'rgba(255,255,255,0.45)', anim:'snow-drift 4.2s ease-in-out infinite', delay:'2s',  opacity:0.5, rotate:45 },
  ],
  valentine: [
...
export function resolveTheme(activeKey: string, customCfg: CustomThemeCfg): ResolvedTheme {
  if (activeKey === 'custom') {
  ...
  return PRESETS[activeKey] ?? PRESETS.neon
}
